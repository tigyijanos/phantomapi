...
- protect authoritative business state from speculative reconstruction
...
2. determine whether the affected artifact is authoritative or reconstructible
3. prefer the smallest safe repair over a broad rewrite
4. record the repair attempt in traces, metrics, and incidents when applicable
5. retry the original operation once only when the repair succeeded and retry is safe
6. if the request is still unsafe, return the best contract-shaped error available
